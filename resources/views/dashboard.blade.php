<x-app-layout>
    ...
    <div style="width: 99%" class="ml-2 my-5 relative overflow-x-hidden sm:rounded-lg">
        ...
        <div>
            <x-primary-button class="ml-3 mb-2" type="button" data-drawer-target="drawer-contact-book"
                data-drawer-show="drawer-contact-book" aria-controls="drawer-contact-book">
                {{ __('Add a Book') }}
            </x-primary-button>
        </div>
        <!-- drawer component -->
        <div style="top : 64px;" id="drawer-contact-book"
            class="fixed left-0 z-40 h-screen p-4 overflow-y-auto transition-transform -translate-x-full bg-white w-80 dark:bg-gray-800"
            ...
            <button type="button" data-drawer-hide="drawer-contact-book" aria-controls="drawer-contact-book"
                ...
                @if ($books->count())
                    ...
                    @foreach ($books as $key => $book)
                        <tr class="bg-white dark:bg-gray-800">
                            ...
                            <td class="px-6 py-4">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sequi quod
                                repellat voluptates necessitatibus dignissimos ab qui officia. Placeat reiciendis
                                voluptatum ut quidem perferendis nobis harum? Perferendis quam sed laboriosam similique!
                                {{ $book->created_at->format('Y-m-d') }}
                            </td>
                            <td class="px-6 py-4 text-right">
                                <a href="/books/{{ $book->id }}"
                                    class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit Or
                                    Delete</a>
                            </td>
                        </tr>
                    @endforeach
    ...
    <div style="width: 99%" class="ml-2 my-5 relative overflow-x-hidden shadow-md sm:rounded-lg">
        <p class="ml-3 max-w-2xl mb-6 font-bold text-black lg:mb-8 md:text-lg lg:text-xl dark:text-gray-400">Categories
        </p>
        <!-- drawer init and show -->
        <div>
            <x-primary-button class="ml-3 mb-2" type="button" data-drawer-target="drawer-contact-category"
                data-drawer-show="drawer-contact-category" aria-controls="drawer-contact-category">
                {{ __('Add a category') }}
            </x-primary-button>
        </div>
        <!-- drawer component -->
        <div style="top : 64px;" id="drawer-contact-category"
            class="fixed left-0 z-40 h-screen p-4 overflow-y-auto transition-transform -translate-x-full bg-white w-80 dark:bg-gray-800"
            tabindex="-1" aria-labelledby="drawer-contact-label">
            <h5 id="drawer-label"
                class="inline-flex items-center mb-6 text-base font-semibold text-gray-500 uppercase dark:text-gray-400">
            ...
            <button type="button" data-drawer-hide="drawer-contact-category" aria-controls="drawer-contact-category"
                class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 absolute top-2.5 right-2.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
                <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
                    xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd"
    ...
    <div style="width: 99%" class="ml-2 my-5 relative overflow-x-hidden shadow-md sm:rounded-lg">
        ...
        <div>
            ...
            <x-primary-button class="ml-3 mb-2" type="button" data-drawer-target="drawer-contact"
                data-drawer-show="drawer-contact" aria-controls="drawer-contact">
                {{ __('create group') }}
            </x-primary-button>
        </div>
        <!-- drawer component -->
        <div style="top : 64px;" id="drawer-contact"
            class="fixed left-0 z-40 h-screen p-4 overflow-y-auto transition-transform -translate-x-full bg-white w-80 dark:bg-gray-800"
            tabindex="-1" aria-labelledby="drawer-contact-label">
            <h5 id="drawer-label"
                class="inline-flex items-center mb-6 text-base font-semibold text-gray-500 uppercase dark:text-gray-400">
